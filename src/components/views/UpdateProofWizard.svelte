<script lang="ts">
    ...
    // --- REACTIVE LOGIC ---
    $: selectedProof = proof_to_update_id
        ? proofs.get(proof_to_update_id)
        : null;
    ...
    async function handleSubmit() {
        ...
        if (!spendable_box) {
            errorMessage =
                "The selected proof has no unassigned tokens available to create a new opinion.";
            isLoading = false;
            return;
        }
        ...
        try {
            ...
            if (txId) {
                ...
                errorMessage =
                    "Transaction was submitted, but no transaction ID was returned.";
            }
        } catch (e: any) {
...
<div class="wizard-container">
    ...
    <div class="step-content">
        {#if currentStep === 1}
            ...
            <label for="proof-select"
                >Your Proofs<span class="required">*</span></label
            >
            <select
                id="proof-select"
                class="input"
                bind:value={proof_to_update_id}
                required
            >
                <option value="" disabled
                    >-- Choose one of your proofs --</option
                >
            ...
            {#if proofs.size === 0}
                <p class="help-text error-text">
                    You do not own any reputation proofs to update.
                </p>
            {/if}
            {#if selectedProof}
                {#if spendable_box}
                    <p class="help-text">
                        Tokens available to assign: {spendable_box.token_amount}
                    </p>
                {:else}
                    <p class="help-text error-text">
                        Warning: This proof has no unassigned tokens. You cannot
                        add new opinions.
                    </p>
                {/if}
            {/if}
        ...
        {#if currentStep === 2}
            ...
            <label for="object-assign-input"
                >Object to Evaluate<span class="required">*</span></label
            >
            <input
                id="object-assign-input"
                type="text"
                class="input"
                bind:value={object_to_assign}
                placeholder="URL, another token ID, text, etc."
                required
            />
            <p class="help-text">
                What is this new opinion about? You can point to any on-chain or
                off-chain object.
            </p>
        {/if}
        ...
        {#if currentStep === 3}
            ...
            <label for="token-amount"
                >Amount of Tokens to Assign to this Opinion<span
                    class="required">*</span
                ></label
            >
            <input
                ...
                min="1"
                max={spendable_box?.token_amount}
                step="1"
            />
            {#if spendable_box}
                <p class="help-text">
                    This amount will be deducted from the unassigned supply
                    (Available: {spendable_box.token_amount}).
                </p>
            {/if}
            <div class="polarity-buttons">
                <button
                    class:selected={!is_negative}
                    on:click={() => (is_negative = false)}>👍 Positive</button
                >
                <button
                    class:selected={is_negative}
                    on:click={() => (is_negative = true)}>👎 Negative</button
                >
            </div>
        {/if}
        ...
        {#if currentStep === 4}
            ...
            <label for="content-data"
                >Additional Data (Plain Text or JSON):</label
            >
            <textarea
                id="content-data"
                class="input"
        ...
        {#if currentStep === 5}
            ...
            <div class="form-check">
                ...
                />
                <label for="lock-checkbox" class="form-check-label"
                    >Lock this new opinion (make it immutable)</label
                >
            </div>
        {/if}
        ...
        {#if currentStep === 6}
            ...
            <div class="summary">
                ...
                <p>
                    <strong>New Pointer:</strong>
                    <span>{object_to_assign}</span>
                </p>
                <p>
                ...
                <p>
                    ...
                    <span>{contentString || "Not provided"}</span>
                </p>
                <p>
                    <strong>New Opinion will be locked:</strong>
                    {is_locked ? "Yes" : "No"}
                </p>
            </div>
            {#if isLoading}
    ...
    <div class="wizard-nav">
        <button on:click={prevStep} disabled={currentStep === 1}
        ...
        <button
            ...
            disabled={currentStep === totalSteps ||
                !proof_to_update_id ||
                !spendable_box}>Next &rarr;</button
        >
    </div>
</div>
...
<style>
    ...
    .polarity-buttons button.selected {
        border-color: #fbbf24;
    }
    .summary {
        background: #333;
